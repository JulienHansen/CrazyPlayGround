...
# ── Trajectory parameters (must match PosTrackingEnvCfg) ──────────────────────
...
class CrazyflieController:
    """Per-drone trajectory-tracking position-control RL agent runner."""
    ...
    def control_loop(self):
        """Phase 1: HLC takeoff. Phase 2: NN position-control trajectory tracking."""
        ...
        self.log.info("NN position-control trajectory tracking active.")
        step = 0
        GRACE_PERIOD = 3.0
        ...
        while self.cf.is_connected() and self.running:
            ...
            if (time.time() - nn_start_time > GRACE_PERIOD and z < 0.1) or z > 2.5:
                self.log.error(f"Altitude out of bounds (z={z:.2f} m) — emergency landing")
                self._emergency_land()
                break
            ...
            self.log.info(
                f"Step={step} | "
                f"target={[f'{v:.2f}' for v in desired_pos.tolist()]} | "
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-drone trajectory tracking — position control (PosTrackingEnvCfg)"
    )
    ...
    try:
        ...
        while not (c1.cf.is_connected() and c2.cf.is_connected()):
            if not (c1.running and c2.running):
                failed = [c.uri for c in (c1, c2) if not c.running]
                logger.error(f"Connection failed for: {failed} — aborting")
                return
            if time.time() > deadline:
                logger.error(f"Connection timeout after {CONNECT_TIMEOUT_S} s — aborting")
                return
            time.sleep(0.5)
