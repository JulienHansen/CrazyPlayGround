...
def newest_run_dir(before: set, root: str | None = None):
    ...
    # (e.g. logs/skrl/cartpole_direct vs logs/skrl/hover_robust) and a path sort picks
...
def main():
    ...
    for i, r in enumerate(runs):
        ...
        cmd = [PYTHON, "scripts/skrl/train.py", "--task", TASK, "--headless",
               ...
               "--seed", str(r["seed"])]
        # optional per-run agent entry point, for grids where two runs share an env
        # config but differ in the agent YAML
        if r.get("agent"):
            cmd += ["--agent", r["agent"]]
        ...
        # be picked up instead (observed: a smoke test under another experiment root
        # was credited to a sweep run). train.py prints the root on startup.
        exp_root = None
        try:
