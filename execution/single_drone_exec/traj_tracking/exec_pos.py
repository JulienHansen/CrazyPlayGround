...
class CrazyflieController:
    """Runs a trained trajectory-tracking position-control RL agent on the Crazyflie."""
    ...
    def control_loop(self):
        ...
        # ── Phase 2: NN position control ──────────────────────────────────────
        logger.info("NN position-control trajectory tracking active.")
        step = 0
...
def main():
    parser = argparse.ArgumentParser(
        description="Trajectory tracking — position control (PosTrackingEnvCfg)"
    )
